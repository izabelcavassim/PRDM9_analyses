...
#read species tree:
speciestree_prdm9_old<-read.newick(file="~/Dropbox/PRDM9_evolution_paper/Simulations/data/species_tree_339_changed_replacements_to_old_names.nwk")
...
#### Re-doing presence and absence, tree and PRDM9 data
uncertain_species = c("Eptesicus_fuscus", "Myotis_lucifugus", "Myotis_davidii", "Gopherus_evgoodei", "Cavia_porcellus", "Denticeps_clupeoides", "Phocoena_sinus", "Saimiri_boliviensis", "Callorhinchus_milii", "Erpetoichthys_calabaricus")
                   
#"Trachemys scripta elegans") -> I couldn't find the RNAseq classification for this species

species_to_keep_2 = c(row.names(presence_absence_df_filtered), uncertain_species)

presence_absence_df_filtered = read.csv("~/Dropbox/PRDM9_evolution_paper/Simulations/data/blast_results_presence_absence_genes_all_339_combined_identified_domains_metadata.csv", sep=",", row.names = 1, stringsAsFactors = FALSE)

presence_absence_df_filtered = subset(presence_absence_df_filtered, presence_absence_df_filtered$Species %in% species_to_keep_2)

# Exclude it from the species tree also
#read species tree:
speciestree_prdm9_old<-read.newick(file="~/Dropbox/PRDM9_evolution_paper/Simulations/data/species_tree_339_changed_replacements_to_old_names.nwk")
speciestree_prdm9 = keep.tip(speciestree_prdm9_old, presence_absence_df_filtered$Species)
...
write.tree(speciestree_prdm9,  "~/Dropbox/PRDM9_submission/Main_figures/Figure3/species_tree_184_3species_per_zero_calls_some_uncertain_included.nw")
write.csv(presence_absence_df_filtered, "~/Dropbox/PRDM9_submission/Main_figures/Figure3/blast_results_presence_absence_genes_all_184_combined_identified_domains_metadata.csv")
...
Bayestraits_run <- function(model="XY", tries, pres_abs=presence_absence_df_filtered, tree=speciestree_prdm9, scaling_factor=mean_bl, max_rate=100, gain_rate, prdm9_status=prdm9, pruning=F, zero_incomplete=FALSE, species_root="Petromyzon_marinus Callorhinchus_milii"){
  ...
  for(i in genes){
    ...
    prdm9_status = as.data.frame(prdm9_status)
    prdm9_status$species = rownames(prdm9_status)
    gene2 = as.data.frame(gene2)
    gene2$species = rownames(gene2)
    
    gene_df = merge(prdm9_status, gene2, by.x="species", by.y="species", all.y = T)
    rownames(gene_df) <- gene_df$species
    ...
    #gene_df["Callorhinchus_milii", 1] <- 0  
    gene_df["Arvicanthis_niloticus", 2] <- 1 # mammal
    gene_df["Orycteropus_afer", 2] <- 1 # mammal
    gene_df["Rhincodon_typus", 2] <- 0
    gene_df["Amblyraja_radiata", 2] <- 0
    gene_df["Acipenser_ruthenus", 2] <- 0
    gene_df["Chanos_chanos", 2] <- 0
    gene_df["Gadus_morhua", 2] <- 0
    gene_df["Salmo_trutta", 2] <- 0
    gene_df["Pangasianodon_hypophthalmus", 2] <- 0
    gene_df["Paramormyrops_kingsleyae", 2] <- 0
    #gene_df["Monodelphis_domestica", 2] <- 1
    #gene_df["Monodelphis_domestica", 2] <- 1
    print(gene_df)
    }
    if(zero_incomplete == FALSE){
    #gene_df["Callorhinchus_milii", 1] <- "-"  
    gene_df["Arvicanthis_niloticus", 2] <- 1
    gene_df["Orycteropus_afer", 2] <- 1
    gene_df["Rhincodon_typus", 2] <- "-"
    gene_df["Amblyraja_radiata", 2] <- "-"
    gene_df["Acipenser_ruthenus", 2] <- "-"
    gene_df["Chanos_chanos", 2] <- "-"
    gene_df["Gadus_morhua", 2] <- "-"
    gene_df["Salmo_trutta", 2] <- "-"
    gene_df["Pangasianodon_hypophthalmus", 2] <- "-"
    gene_df["Paramormyrops_kingsleyae", 2] <- "-"
    gene_df["Monodelphis_domestica", 2] <- 0
    
    
    # Other species that we are not certain about their PRDM9 values
    gene_df["Eptesicus_fuscus", 2] <- "-"
    gene_df["Myotis_lucifugus", 2] <- "-"
    gene_df["Myotis_davidii", 2] <- "-"
    gene_df["Gopherus_evgoodei", 2] <- "-"
    gene_df["Cavia_porcellus", 2] <- "-"
    gene_df["Denticeps_clupeoides", 2] <- "-"
    gene_df["Phocoena_sinus", 2] <- "-"
    gene_df["Saimiri_boliviensis", 2] <- "-"
    gene_df["Callorhinchus_milii", 2] <- "-"
    gene_df["Erpetoichthys_calabaricus",2] <- "-"
    #gene_df["Trachemys scripta elegans", 1] <- "-"
    }
    ...
    binary = TRUE
    if(sum(as.numeric(gene_df[,3])) == nrow(gene_df)){
    ...
    if(binary == TRUE){
      ...
      colnames(gene_df) <- c("species", "prdm9", "gene2")
...
library(qvalue)
qobj <- qvalue(p = p, fdr.level = 0.5, lfdr.out = F)
